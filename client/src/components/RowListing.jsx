...
const RowListing = ({ title, movies }) => {
  ...
  return (
    <div className="row">
     ...
     {movies.map(movie=>(<Movie key={movie.id} src ={movie.source} className ="movie" alt ={movie.name}/>))}
      </ul >
      <button onClick={scrollRight} style={{marginLeft:"5px"}} className="row-button">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          fill="none"
          role="img"
